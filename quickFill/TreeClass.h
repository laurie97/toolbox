...
// Mon Aug  3 11:21:18 2015 by ROOT version 6.02/05
// from TTree outTree/outTree
// found on file: /atlas/data5/userdata/guescini/dijet/inputs/MC15a_20150707/tree/user.jdandoy.mc15_13TeV.361026.Pythia8EvtGen_A14NNPDF23LO_jetjet_JZ6W.AllUp1-0.MC15a_20150707_tree.root
//////////////////////////////////////////////////////////
...
class TreeClass {
  ...
  */
  ...
  //Float_t         mbb_flt_6060;
  //Float_t         mbb_flt_7070;
  Float_t         mbb_flt_7777;
  Float_t         mbb_flt_8585;
  /*
  ...
  */
  ...
  //TBranch        *b_mbb_flt_6060;   //!
  //TBranch        *b_mbb_flt_7070;   //!
  TBranch        *b_mbb_flt_7777;   //!
  TBranch        *b_mbb_flt_8585;   //!
  /*
  ...
  */
  ...
  TreeClass(TTree *tree=0, bool isMC=false);
  virtual ~TreeClass();
  ...
  void test();
  Long64_t readWhatYouNeed(Long64_t entry);
  void setProcess(string process) {m_process = process;};
...
//******************************************
TreeClass::TreeClass(TTree *tree, bool isMC) : fChain(0), m_isMC(0), m_hasMbb(0)
{
  // if parameter tree is not specified (or zero), connect the file
  // used to generate this class and read the Tree.
  if (tree == 0) {
    TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/atlas/data5/userdata/guescini/dijet/inputs/MC15a_20150707/tree/user.jdandoy.mc15_13TeV.361026.Pythia8EvtGen_A14NNPDF23LO_jetjet_JZ6W.AllUp1-0.MC15a_20150707_tree.root");
    if (!f || !f->IsOpen()) {
      f = new TFile("/atlas/data5/userdata/guescini/dijet/inputs/MC15a_20150707/tree/user.jdandoy.mc15_13TeV.361026.Pythia8EvtGen_A14NNPDF23LO_jetjet_JZ6W.AllUp1-0.MC15a_20150707_tree.root");
    }
...
//******************************************
//NOTE this is rather slow, use readWhatYouNeed(entry) instead
...
//******************************************
//NOTE this is a faster alternative to GetEntry(entry) which loads the entry of all the branches of the tree
Long64_t TreeClass::readWhatYouNeed(Long64_t entry)
{
  ...
  if (m_isMC) {
    b_mcEventNumber->GetEntry(entry);
    b_mcChannelNumber->GetEntry(entry);
    b_mcEventWeight->GetEntry(entry);
  }

  b_yStar->GetEntry(entry);
  b_yBoost->GetEntry(entry);
  b_mjj->GetEntry(entry);

  if (m_hasMbb) {
    b_mbb_fix_8585->GetEntry(entry);
    b_mbb_fix_7777->GetEntry(entry);
    //b_mbb_fix_7070->GetEntry(entry);
    //b_mbb_fix_6060->GetEntry(entry);
    
    b_mbb_flt_8585->GetEntry(entry);
    b_mbb_flt_7777->GetEntry(entry);
    //b_mbb_flt_7070->GetEntry(entry);
    //b_mbb_flt_6060->GetEntry(entry);
  }
  
  b_weight->GetEntry(entry);
  b_weight_xs->GetEntry(entry);
  b_weight_prescale->GetEntry(entry);

  b_passedTriggers->GetEntry(entry);

  b_jet_E->GetEntry(entry);
  b_jet_pt->GetEntry(entry);
  b_jet_phi->GetEntry(entry);
  b_jet_eta->GetEntry(entry);
  b_jet_rapidity->GetEntry(entry);

  b_jet_NumTrkPt1000PV->GetEntry(entry);
  b_jet_SumPtTrkPt1000PV->GetEntry(entry);
  b_jet_TrackWidthPt1000PV->GetEntry(entry);
  b_jet_NumTrkPt500PV->GetEntry(entry);
  b_jet_SumPtTrkPt500PV->GetEntry(entry);
  b_jet_TrackWidthPt500PV->GetEntry(entry);

  b_jet_MV2c00->GetEntry(entry);
  b_jet_MV2c20->GetEntry(entry);

  if (m_isMC) {
    b_jet_truth_pt->GetEntry(entry);
  }
...
void TreeClass::Init(TTree *tree)
{
  ...
  //check if tree contains some branches
  if (tree->GetListOfBranches()->FindObject("mbb_fix_8585"))
    m_hasMbb=true;
  ...
  */
  if (m_hasMbb) {
    ...
    //fChain->SetBranchAddress("mbb_flt_6060", &mbb_flt_6060, &b_mbb_flt_6060);
    //fChain->SetBranchAddress("mbb_flt_7070", &mbb_flt_7070, &b_mbb_flt_7070);
    fChain->SetBranchAddress("mbb_flt_7777", &mbb_flt_7777, &b_mbb_flt_7777);
    fChain->SetBranchAddress("mbb_flt_8585", &mbb_flt_8585, &b_mbb_flt_8585);
  }
  /*
  ...
  */
  ...
  /*
    fChain->SetBranchAddress("passAny", &passAny, &b_passAny);
